...
% --- locate data/ robustly ---------------------------------------------------
% This script may be launched from data/ directly, OR via the app's
% runScript() which calls run('.../scripts/verify_outputs.m'); MATLAB's run()
% cd's into the script's own folder (scripts/) before executing, so bare
% filenames would resolve against scripts/ (where no .mat exist) and everything
% would wrongly report MISSING. Self-locate the data dir the same way
% generate_reference_surface.m does, and prefix every file op with DATA_DIR.
VERIFY_SCRIPT_DIR = fileparts(mfilename('fullpath'));         % .../scripts
DATA_DIR = fullfile(fileparts(VERIFY_SCRIPT_DIR), 'data');   % .../data
if ~exist(DATA_DIR, 'dir'); DATA_DIR = pwd; end              % standalone fallback
